...
const App = () => {
    ...
    // Ref to hold the interval ID for monitoring to clear it later
    ...
    const handleVersionModel = () => {
        ...
        setTimeout(() => {
            ...
            setCurrentModelVersion(modelVersionDetails);
    ...
    const handlePredict = () => {
        ...
        // Simulate incrementing prediction count for monitoring
        setMonitoringData(prev => ({ ...prev, predictionCount: prev.predictionCount + 1 }));
        ...
        setTimeout(() => {
            ...
            if (predictionInput.toLowerCase().includes('good') || predictionInput.toLowerCase().includes('positive') || predictionInput.toLowerCase().includes('happy')) {
                simulatedScore = (0.7 + Math.random() * 0.3).toFixed(2);
                ...
                simulatedScore = (0.1 + Math.random() * 0.2).toFixed(2);
            } else {
                simulatedScore = (0.4 + Math.random() * 0.2).toFixed(2);
            }
     ...
     * 1. Collecting metrics from the deployed endpoint (e.g., latency, throughput, error rates).
     * 2. Analyzing incoming prediction requests and comparing them to training data for data drift.
     * 3. Analyzing model predictions over time for model drift (e.g., performance degradation).
     * 4. Logging these metrics to a monitoring system (e.g., Cloud Monitoring, Prometheus).
     */
    const handleStartMonitoring = () => {
        if (!isModelDeployed) {
            setMonitoringStatus('Please deploy a model first to start monitoring.');
            return;
        }
        if (isMonitoringActive) {
            setMonitoringStatus('Monitoring is already active.');
            return;
        }

        setIsMonitoringActive(true);
        setMonitoringStatus('Monitoring active. Simulating real-time metrics...');
        setAlertMessage(''); // Clear any previous alerts

        // Simulate metric updates every 2 seconds
        monitoringIntervalRef.current = setInterval(() => {
            setMonitoringData(prev => {
                const newLatency = parseFloat((50 + Math.random() * 100).toFixed(1)); // 50-150ms
                const newDataDrift = parseFloat((Math.random() * 0.6).toFixed(2)); // 0.0 - 0.6
                const newModelDrift = parseFloat((Math.random() * 0.05).toFixed(2)); // 0.0 - 0.05
                const newErrorRate = parseFloat((Math.random() * 0.01).toFixed(3)); // 0.0 - 0.01

                // Simulate occasional drift for alerting
                let currentDataDrift = newDataDrift;
                if (prev.predictionCount > 5 && Math.random() < 0.3) { // 30% chance to increase drift after 5 predictions
                    currentDataDrift = parseFloat((0.7 + Math.random() * 0.3).toFixed(2)); // High drift 0.7-1.0
                }

                return {
                    ...prev,
                    avgLatencyMs: newLatency,
                    dataDriftScore: currentDataDrift,
                    modelDriftScore: newModelDrift,
    ...
    /**
     * Effect to check for alerts based on monitoring data.
     * In a real system, this would be handled by Cloud Monitoring alerts,
     * Prometheus Alertmanager, or custom alerting services.
     */
    useEffect(() => {
        if (isMonitoringActive) {
            const DRIFT_THRESHOLD = 0.7; // Example threshold for data drift
            if (monitoringData.dataDriftScore > DRIFT_THRESHOLD) {
                setAlertMessage(`🚨 ALERT: High Data Drift Detected! Score: ${monitoringData.dataDriftScore}. Consider retraining model.`);
            } else {
                setAlertMessage(''); // Clear alert if drift is below threshold
            }
        }
    }, [monitoringData, isMonitoringActive]);
    ...
    const resetPipeline = () => {
        ...
        // Reset all data pipeline states
        setCurrentRawData('');
        setIngestionStatus('');
        ...
        // Reset all model pipeline states
        setTrainedModelId('');
        setTrainingStatus('');
        ...
        // Reset all deployment & prediction states
        ...
        // Reset all monitoring & alerting states
        setMonitoringData({
            predictionCount: 0,
            avgLatencyMs: 0,
            dataDriftScore: 0,
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4 font-sans">
            ...
            <style>
                ...
                .grid-cols-2-uneven { grid-template-columns: 2fr 3fr; }
                .gap-6 { gap: 1.5rem; }
                ...
                .hover\\:bg-blue-grey-700:hover { background-color: #37474f; }
                ...
                .bg-lime-600 { background-color: #9e9d24; }
                .hover\\:bg-lime-700:hover { background-color: #827717; }
                .bg-lime-100 { background-color: #f0f4c3; }
                .text-lime-800 { color: #827717; }

                .bg-cyan-50 { background-color: #e0f7fa; }
                .border-cyan-200 { border-color: #b2ebf2; }
                .text-cyan-700 { color: #00838f; }
                .bg-cyan-600 { background-color: #00acc1; }
                .hover\\:bg-cyan-700:hover { background-color: #00838f; }
                .bg-cyan-100 { background-color: #b2ebf2; }
                .text-cyan-800 { color: #006064; }
                `}
            </style>

            <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-6xl">
                <h1 className="text-3xl font-bold text-gray-800 mb-6 text-center">
                    Full MLOps Pipeline: Data to Monitoring & Alerting (Conceptual)
                </h1>
                ...
                {isAuthReady && (
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-2-uneven gap-6">
                            {/* Left Column: Data & Model Development */}
                            ...
                            {/* Right Column: Deployment, Prediction, Monitoring */}
                            <div className="space-y-6">
                                <h2 className="text-2xl font-bold text-gray-700 mb-4 col-span-full">Model Deployment & Operations</h2>
                                {/* 8. Model Deployment */}
                                <div className="p-6 bg-blue-grey-50 rounded-lg border border-blue-grey-200 shadow-sm">
                                ...
                                {/* 9. Make Predictions */}
                                <div className="p-6 bg-lime-50 rounded-lg border border-lime-200 shadow-sm">
                                ...
                                <div className="p-6 bg-cyan-50 rounded-lg border border-cyan-200 shadow-sm">
                                    <h3 className="text-xl font-semibold text-cyan-700 mb-3">10. Monitoring & Alerting</h3>
                                    <div className="flex space-x-2 mb-3">
                                        <button
                                            onClick={handleStartMonitoring}
                                            className="flex-1 bg-cyan-600 text-white py-2 px-4 rounded-md hover:bg-cyan-700 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                                            disabled={!isModelDeployed || isMonitoringActive}
                                        >
                                            Start Monitoring
                                        </button>
                                        <button
                                    ...
                                    {monitoringStatus && <p className={`mt-2 text-xs font-medium ${monitoringStatus.includes('active') ? 'text-green-600' : 'text-gray-600'}`}>{monitoringStatus}</p>}
                                    {isMonitoringActive && (
                                        <div className="mt-3 p-2 bg-cyan-100 border border-cyan-200 rounded-md text-cyan-800 text-xs">
                                            <p className="font-semibold mb-1">Live Metrics:</p>
                                            <ul className="list-disc list-inside">
                                                <li><strong>Predictions:</strong> {monitoringData.predictionCount}</li>
                                                <li><strong>Avg Latency:</strong> {monitoringData.avgLatencyMs} ms</li>
                                                <li><strong>Data Drift:</strong> {monitoringData.dataDriftScore} (Threshold: 0.7)</li>
                                                <li><strong>Model Drift:</strong> {monitoringData.modelDriftScore}</li>
                                                <li><strong>Error Rate:</strong> {(monitoringData.errorRate * 100).toFixed(2)}%</li>
                                            </ul>
                                        </div>
                                    )}
                                    {alertMessage && (
                                        <div className="mt-3 p-3 bg-red-100 border border-red-400 text-red-700 rounded-md text-sm font-bold animate-pulse">
                                            {alertMessage}
                                        </div>
                                    )}
                                </div>
